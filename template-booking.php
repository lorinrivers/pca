...
if(isset($_POST['submitted'])) {
	...
	if(trim($_POST['checking']) !== '') {
		...
		//Check to make sure that the contact number field is not empty
		if(trim($_POST['contactNumber']) === '') {
			$numberError =  __('You forgot to enter your Contact Number.', 'woothemes'); 
			$hasError = true;
		} else {
			$number = trim($_POST['contactNumber']);
		}
		
		//Check to make sure Address was filled in	
		if(trim($_POST['contactAddress']) === '') {
			$addressError = __('You forgot to enter your Contact Address.', 'woothemes');
			$hasError = true;
		} else {
			if(function_exists('stripslashes')) {
				$address = stripslashes(trim($_POST['contactAddress']));
			} else {
				$address = trim($_POST['contactAddress']);
			}
		}
		
		//Check to make sure that the event field is not empty
		if(trim($_POST['eventName']) === '') {
			$eventError =  __('You forgot to enter the Event name.', 'woothemes'); 
			$hasError = true;
		} else {
		...
		if(!isset($hasError)) {
			...
			$emailTo = get_option('woo_bookingform_email'); 
			$subject = __('Booking Form Submission from ', 'woothemes').$name;
		...
		<div id="main" class="col-left">
            ...
            <?php } else { ?>
                ...
                <?php while (have_posts()) : the_post(); ?>
                    ...
                    <?php if ( get_option('woo_bookingform_email') == '' ) { ?>
                        <p class="alert"><?php _e('E-mail has not been setup properly. Please add your booking e-mail!', 'woothemes'); ?></p>
                    <?php } ?>
                    ...
                    <form action="<?php the_permalink(); ?>" id="contactForm" method="post">
                        ...
                        <ol class="forms">
                            ...
                            <li><label for="contactNumber"><?php _e('Phone Number', 'woothemes'); ?></label>
                                <input type="text" name="contactNumber" id="contactNumber" value="<?php if(isset($_POST['contactNumber'])) echo $_POST['contactNumber'];?>" class="txt requiredField" />
                                <?php if($numberError != '') { ?>
                                    <span class="error"><?php echo $numberError;?></span> 
                                <?php } ?>
                            </li>
                            
                            <li class="textarea"><label for="contactAddress"><?php _e('Address', 'woothemes'); ?></label>
                                <textarea name="contactAddress" id="contactAddress" rows="20" cols="30" class="requiredField"><?php if(isset($_POST['contactAddress'])) { if(function_exists('stripslashes')) { echo stripslashes($_POST['contactAddress']); } else { echo $_POST['contactAddress']; } } ?></textarea>
                                <?php if($addressError != '') { ?>
                                    <span class="error"><?php echo $addressError;?></span> 
                                <?php } ?>
                            </li>
                            
                            <li><label for="eventName"><?php _e('Event Name', 'woothemes'); ?></label>
                                <input type="text" name="eventName" id="eventName" value="<?php if(isset($_POST['eventName'])) { echo $_POST['eventName']; } elseif (isset($_GET['event_id'])) { $woo_post = get_post($_GET['event_id']); if ($woo_post) { echo $woo_post->post_title; } } ?>" class="txt requiredField" />
                                <?php if($eventError != '') { ?>
                            ...
                            <li class="textarea"><label for="commentsText"><?php _e('Message', 'woothemes'); ?></label>
                                <textarea name="comments" id="commentsText" rows="20" cols="30" class="requiredField"><?php if (isset($_GET['event_id'])) { $woo_post = get_post($_GET['event_id']); if ($woo_post) { echo "I'm interested in attending ".$woo_post->post_title; } } ; ?><?php if(isset($_POST['comments'])) { if(function_exists('stripslashes')) { echo stripslashes($_POST['comments']); } else { echo $_POST['comments']; } } ?></textarea>
                                <?php if($commentError != '') { ?>
